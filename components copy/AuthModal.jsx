'use client'

import { signIn } from 'next-auth/react'

/**
 * AuthModal Component - Google OAuth Version
 * Modal for user authentication using Google Sign In
 * 
 * Features:
 * - One-click Google authentication
 * - Automatic account creation on first sign in
 * - Session persists across devices
 * - Clean, simple UI
 */

export default function AuthModal({ isOpen, onClose }) {
  if (!isOpen) return null

  const handleGoogleSignIn = async () => {
    try {
      await signIn('google', {
        callbackUrl: window.location.href, // Stay on current page after sign in
        redirect: true,
      })
    } catch (error) {
      console.error('Sign in error:', error)
    }
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-surface-container rounded-3xl shadow-premium-lg max-w-md w-full">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-outline-variant">
          <h2 className="text-title-large font-semibold text-on-surface">
            Sign In
          </h2>
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full hover:bg-surface-container-high transition-colors flex items-center justify-center"
          >
            <svg className="w-6 h-6 text-on-surface" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        </div>

        {/* Content */}
        <div className="p-6 space-y-6">
          <div className="text-center">
            <p className="text-body-large text-on-surface-variant mb-6">
              Sign in with your Google account to continue
            </p>
          </div>

          {/* Google Sign In Button */}
          <button
            onClick={handleGoogleSignIn}
            className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-white border-2 border-outline-variant rounded-full hover:bg-surface-container-high hover:border-outline transition-all shadow-elevation-1 hover:shadow-elevation-2"
          >
            <svg className="w-6 h-6" viewBox="0 0 24 24">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            <span className="text-label-large font-medium text-on-surface">
              Continue with Google
            </span>
          </button>

          <p className="text-body-small text-on-surface-variant text-center">
            By signing in, you agree to our Terms of Service and Privacy Policy
          </p>
        </div>
      </div>
    </div>
  )
}
